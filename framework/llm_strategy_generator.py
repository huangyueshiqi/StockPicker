...
class GlobalParams(BaseModel):
    ...
    lookback_years:int = Field(default=1, description="数据回看窗口(年)，用于需要历史窗口计算的指标(如3年CAGR)")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prompt", type=str, help="策略的自然语言描述")
    parser.add_argument("--output", type=str, default="config/generated_strategy.json", help="输出的 JSON 配置文件路径")
    parser.add_argument("--model", type=str, default="kimi", help="使用的模型名称")
    parser.add_argument("--base-url", type=str, default="http://172.21.16.9/ms-tnpb4z5j/v1", help="LLM API Base URL")
    parser.add_argument("--api_key", type=str, default="app-kKH20nKvnhRhRzoAZSMWhFVJ", help="LLM API KEY")

    args = parser.parse_args()

    generator = LLMStrategyGenerator(model=args.model, base_url=args.base_url,api_key=args.api_key)
    prompt="我想要一个低市盈率的价值策略，选出行业内市盈率最低的20只股票，调仓周期为30天，测试时间从2021年1月1日到2023年12月31日"
    # 1. 生成策略配置
    # generator.generate(prompt, args.output)
    # strategy_config = generator.generate(prompt, args.output)
    with open("config/generated_strategy.json",'r',encoding='utf-8') as f:
        strategy_config=json.load(f)

    if strategy_config:
        # 2. 如果存在本地 CSV 对照表，则自动进行字段映射
        csv_files = [
            Path(f"{PROJECT_ROOT.parent}/documents/财务字段中英文对照表.csv"),
            Path(f"{PROJECT_ROOT.parent}/documents/宏观微观字段名对照表.csv"),
            Path(f"{PROJECT_ROOT.parent}/documents/量价字段中英文对照表.csv")
        ]
