...
class QlibPremiumValueStrategy(BaseStrategy):
    ...
    def __init__(self,
        ...
        if use_config:
            ...
            lookback_years=global_params.get("lookback_years",1)
...
def main(args):
    ...
    if not prompt:
        if args.interactive:
            ...
            prompt = "回测区间：2021-01-01到2025-12-31。寻找低市盈率(PE_TTM)且高ROE的股票，每60个交易日调仓，等权持仓，选前20只"  # 默认 fallback
...
#权重方式，市值加权，特征加权，选股数量，调仓周期，参数组合
    # 初始化 LLM 生成器
    # 这里的模型和 base_url 可根据需要更改，或者依赖环境变量 OPENAI_API_KEY
    # generator = LLMStrategyGenerator(model="Kimi-K2.6", base_url="http://172.21.16.9/ms-tnpb4z5j/v1",api_key="app-kKH20nKvnhRhRzoAZSMWhFVJ")
    # generator = LLMStrategyGenerator(model="Kimi-K2.6", base_url="http://10.9.12.101:30002/v1",
    #                                  api_key="app-kKH20nKvnhRhRzoAZSMWhFVJ")
    generator = LLMStrategyGenerator(model="deepseek-v3", base_url="http://172.21.16.9/ms-r6rcvnnp/v1",
                                     api_key="app-kKH20nKvnhRhRzoAZSMWhFVJ")
    # generator = LLMStrategyGenerator(model="qwen3.5-35b", base_url="http://10.6.60.60:8123/v1",
    #                                  api_key="any-string-is-fine")
    ...
    if need_generate_config or not os.path.exists(mapping_file):
        ...
        print("发现已缓存的字段映射 mapping_result.json，直接复用。cache_id={cache_id}")
